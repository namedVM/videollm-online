...
class LiveMixin(AutoModelForCausalLM):
    def set_vision_inside(self):
        logger.warning_once(
            "!!! Set vision encoder in the model, only recommended for in-the-wild inference. "
    ...
    def stream_evaluate(
        ...
        **kwargs,
        ...
        turn_stops = ((input_id == self.config.eos_token_id).nonzero() + 1)[
            :, 0
        ].tolist()
        ...
        outputs = self.forward(
            input_ids=input_ids, frames=frames, return_dict=True, use_cache=True
        )
        ...
        frame_token_interval_id = (
            self.config.frame_token_interval_id
            if use_interval
        ...
        if self.config.frame_token_pooled:
            frame_num_tokens += (
                self.config.frame_token_pooled[0] * self.config.frame_token_pooled[1]
            )
        past_num_frames = 0
        ...
        for r, (turn_start, turn_stop) in enumerate(zip(turn_starts, turn_stops)):
            ...
            if turn_lm_mask.any():
                turn_lm_masked_logit, turn_lm_masked_label = (
                    turn_logit[turn_lm_mask],
                    turn_label[turn_lm_mask],
                )
                ...
                lm_correctness.append(
                    num_lm_correct_tokens / turn_lm_masked_label.numel()
                )
            ...
            if turn_stream_mask.any():
                ...
                turn_stream_masked_pred_mask = (
                    turn_stream_masked_score.argmax(dim=-1) != frame_token_interval_id
                )
                if turn_stream_masked_pred_mask.any():
                    frame_diff = (
                        turn_stream_mask.sum()
                        - turn_stream_masked_pred_mask.nonzero()[0, 0]
                        - 1
                    )
                else:
                    ...
                    past_key_values_before_assistant = self.trim_past_key_values(
                        past_key_values, 0, turn_start + turn_last_stream_idx + 1
                    )
                    if (
                        r == num_turns - 1
                    ):  # no future frame. we assume the model should receive a signal when streaming ends (e.g. close button).
                        frame_diff = zero
                    else:
                        next_turn_num_frames = (
                            input_id[turn_starts[r + 1] : turn_stops[r + 1]]
                            == v_placeholder_id
                        ...
                        if to_append_num_frames == 0:
                            ...
                            to_append_frames = frames[
                                past_num_frames + turn_num_frames : past_num_frames
                                + turn_num_frames
                            ...
                            to_append_score_pred_mask = (
                                to_append_score.argmax(dim=-1)
                                != frame_token_interval_id
                            )
                            if to_append_score_pred_mask.any():
                                frame_diff = -(
                                    to_append_score_pred_mask.nonzero()[0, 0] + 1
                                )
                            else:
            ...
            if turn_lm_mask.any() and turn_stream_mask.any():
                ...
                num_learn_valid_tokens = (
                    turn_lm_masked_label.numel() + num_learn_v_tokens
                )
                if frame_diff == 0:
                    fluency = (
                        num_learn_v_tokens + num_lm_correct_tokens
                    ) / num_learn_valid_tokens
                elif frame_diff > 0:
        ...
        lm_correctness = (
            torch.stack(lm_correctness).float().mean() if lm_correctness else one
        )
    ...
    if is_training:
        ...
        if resume_from_checkpoint:
            model = PeftModel.from_pretrained(
                model, resume_from_checkpoint, is_trainable=False
            )
        else:
            logger.warning(
                f"!!! Fail to load checkpoint: {resume_from_checkpoint}. Return a new initialized model."
            )
