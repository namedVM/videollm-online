...
def build_live_vision(config: LiveConfigMixin):
    ...
    if "google/siglip-large-patch16-384" == config.vision_pretrained:
        return model, partial(
            _siglip_vision_encode,
            frame_token_cls=config.frame_token_cls,
            frame_token_pooled=config.frame_token_pooled,
        )
    elif (
        "laion/CLIP-ViT-L-14-DataComp.XL-s13B-b90k" == config.vision_pretrained
        or "openai/clip-vit-large-patch14-336" == config.vision_pretrained
    ):
