...
def get_stream_placeholder_len(num_frames: int, model_config: LiveConfigMixin) -> str:
    return num_frames * model_config.frame_num_tokens * len(
...
def get_stream_learn_ranges(
    ...
    len_frame_placeholder_with_interval = model_config.frame_num_tokens * len(
        model_config.v_placeholder
    ...
    learn_ranges = torch.stack(
        [intermediate_interval_idxs, intermediate_interval_idxs + len_learn], dim=1
    )
    return learn_ranges
...
def get_learn_ranges(
    ...
    *,
    chat_template_offsets: dict[tuple, int],
    ...
    for message in conversation:
        ...
        if role == "stream":
            if message.get("learn", False):
                ranges = (
                    get_stream_learn_ranges(message["num_frames"], model_config)
                    + offset
                )
            ...
            if role == "assistant":
                if message.get("learn", False):
                    learn_ranges.append(
                        range(
                            offset - chat_template_offsets["assistant"],
                            offset
                            + len(message["content"])
                            + chat_template_offsets["eos_token"],
                        )
                    )
...
def build_live_tokenizer_and_update_config(
    llm_pretrained: str, model_config: LiveConfigMixin
) -> AutoTokenizer:
    tokenizer = AutoTokenizer.from_pretrained(
        llm_pretrained, use_fast=True, padding_side="left"
    )
    tokenizer.add_special_tokens(
        {"additional_special_tokens": [model_config.v_placeholder]}
    )
    v_placeholder_id = len(tokenizer) - 1
    if model_config.frame_token_interval:
        frame_token_interval_id = tokenizer.convert_tokens_to_ids(
...
if __name__ == "__main__":
    ...
    tokenizer = build_live_tokenizer_and_update_config(
        "meta-llama/Meta-Llama-3-8B-Instruct", config
    )
    chat = [
