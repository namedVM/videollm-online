...
if __name__ == "__main__":
    ...
    compute_metrics_dict = get_compute_metrics_dict(
        dataset_dict=eval_dataset_dict, tokenizer=tokenizer
    )
        ...
        for dataset_name, dataset in eval_dataset_dict.items():
            ...
            for i, batch in enumerate(tqdm.tqdm(dl, desc=f"debug run for evaluation")):
                ...
                dummy_predictions.append(
                    torch.tensor(
                        tokenizer(
                            "\n".join(dataset.labels[batch.sample_idxs[0]])
                        ).input_ids[1:]
                    )
                )
                label_ids.append(batch.sample_idxs)
            print(
                compute_metrics_dict[dataset_name](
                    transformers.EvalPrediction(
                        predictions=torch.nn.utils.rnn.pad_sequence(
                            dummy_predictions, batch_first=True, padding_value=-100
                        ).numpy(),
