...
def create_board(
    ...
    if assistant_logo:
        ...
        resized_assistant_logo = assistant_logo.resize(
            (assistant_logo_width, assistant_logo_height)
        )
...
def append_text(board: dict, text: str, with_logo: str = None):
    wrapped_lines = textwrap.wrap(
        text, width=board["max_chars"], break_long_words=True, replace_whitespace=True
    )
    for i, line in enumerate(wrapped_lines):
        board["last"] += np.array([0, board["interval"]])
        if with_logo and i == 0:
            board["image"].paste(
                board[with_logo], board["last"].tolist(), board[with_logo]
            )
            board["draw"].text(
                (board["last"][0] + board[with_logo].width, board["last"][1]),
                line,
                fill=(255, 255, 255),
                font=board["font"],
            )
            ...
            board["draw"].text(
                board["last"], line, fill=(255, 255, 255), font=board["font"]
            )
...
def create_live_boards(height, user_logo, assistant_logo, font_path, width=500):
    state_board = create_board(
        height=30,
        width=width,
        font_path=font_path,
        font_size=20,
        interval=10,
        initial_text="Waiting...",
    )
    conversation_board = create_board(
        user_logo=user_logo,
        assistant_logo=assistant_logo,
        height=height - 30,
        width=width,
        font_path=font_path,
        font_size=20,
...
def main(
    ...
    height = 540
    width = int(
        height * cap.get(cv2.CAP_PROP_FRAME_WIDTH) / cap.get(cv2.CAP_PROP_FRAME_HEIGHT)
    )
    ...
    out = cv2.VideoWriter(
        "demo/rendering/temp.mp4", fourcc, video_fps, (width + board_width, height)
    )
    ...
    for frame_idx in range(19260817):
        ...
        if frame_idx == 0:
            state_board, conversation_board = create_live_boards(
                height,
                width=board_width,
                user_logo=user_logo,
                assistant_logo=assistant_logo,
        ...
        update_text(
            state_board,
            f"Video Time = {video_time:.1f}s, Average Processing FPS = {inference_fps:.1f}, GPU: RTX 3090",
        )
        while (
            len(conversation) > message_idx
            and video_time >= conversation[message_idx]["time"]
        ):
            ...
            if "role" in message:
                ...
                if role == "user":
                    append_text(
                        conversation_board, message["content"], with_logo="user_logo"
                    )
                    streaming_texts.append(
                        (video_time, message["en"], user_infer_code, 24000)
                    )
                elif role == "assistant":
                    append_text(
                        conversation_board,
                        message["content"],
                        with_logo="assistant_logo",
                    )
                    streaming_texts.append(
                        (video_time, message["en"], assistant_infer_code, 28000)
                    )
            message_idx += 1
