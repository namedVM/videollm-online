    ...
    with gr.Row():
        with gr.Column():
            ...
            gr_examples = gr.Examples(
                examples=[
                    ["demo/assets/cooking.mp4"],
                    ["demo/assets/bicycle.mp4"],
                    ["demo/assets/egoexo4d.mp4"],
                ],
                inputs=gr_video,
                outputs=gr_video,
                label="Examples",
            ...
            gr.Markdown("## Tips:")
            gr.Markdown(
                "- When you upload/click a video, the model starts processing the video stream. You can input a query before or after that, at any point during the video as you like."
            )
            gr.Markdown(
                "- **Gradio refreshes the chatbot box to update the answer, which will delay the program. If you want to enjoy faster demo as we show in teaser video, please use https://github.com/showlab/videollm-online/blob/main/demo/cli.py.**"
            )
            gr.Markdown(
                "- This work is primarily done at a university, and our resources are limited. Our model is trained with limited data, so it may not solve very complicated questions. However, we have seen the potential of 'learning in streaming'. We are working on new data method to scale streaming dialogue data to our next model."
            )
        ...
        with gr.Column():
            gr_chat_interface = gr.ChatInterface(
                ...
                examples=[
                    ...
                    "Hi, guide me the next step.",
                ],
            )

            def gr_frame_token_interval_threshold_change(
                frame_token_interval_threshold,
            ):
                liveinfer.frame_token_interval_threshold = (
                    frame_token_interval_threshold
                )

            gr_frame_token_interval_threshold = gr.Slider(
                minimum=0,
                maximum=1,
                step=0.05,
                value=liveinfer.frame_token_interval_threshold,
                interactive=True,
                label="Streaming Threshold",
            )
            gr_frame_token_interval_threshold.change(
                gr_frame_token_interval_threshold_change,
                inputs=[gr_frame_token_interval_threshold],
            )
        ...
        def gr_video_change(src_video_path, history, video_time, gate):
            ...
            ffmpeg_video_path = os.path.join(
                "demo/assets/cache",
        ...
        gr_video_time.change(
            gr_video_time_change,
            [gr_video, gr_video_time],
            [gr_video_time],
            js=get_gr_video_current_time,
        )
        ...
        gr_liveinfer_queue_refresher.change(
            gr_liveinfer_queue_refresher_change,
            inputs=[gr_chat_interface.chatbot],
