...
def distributed_refine_narration(args: LiveOnePlusEncodingArguments):
    ...
    model = AutoModelForCausalLM.from_pretrained(
        args.llm_pretrained, torch_dtype="auto", attn_implementation="sdpa"
    )
    ...
    anno_path = os.path.join(
        args.ego4d_anno_root, f"narration_stream_{args.split}.json"
    )
    save_dir = os.path.join(
        args.ego4d_anno_root, f"refined_narration_stream_{args.split}"
    )
    ...
    annos = {
        video_uid: _annotation_uid_narrations
        for i, (video_uid, _annotation_uid_narrations) in tqdm.tqdm(
            enumerate(annos.items())
        )
        if not os.path.exists(os.path.join(save_dir, f"{video_uid}.json"))
    }
    for i, (video_uid, _annotation_uid_narrations) in tqdm.tqdm(
        enumerate(annos.items())
    ):
                    ...
                    chat = [
                        {
                            "role": "user",
                            "content": (
                                "Please help me to refine the text, e.g., [C looks around.] -> [You look around.]"
