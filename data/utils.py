...
def ffmpeg_once(
    ...
    *,
    ...
    if resolution is not None:
        command += [
            "-vf",
            f"scale='if(gt(iw\\,ih)\\,{resolution}\\,-2)':'if(gt(iw\\,ih)\\,-2\\,{resolution})',pad={resolution}:{resolution}:(ow-iw)/2:(oh-ih)/2:color='{pad}'",
        ]
    command += [dst_path]
    subprocess.run(command, check=True)


def distributed_ffmpeg(
    *,
    src_root: str,
    fps: int = None,
    resolution: int = None,
    ...
    src_paths = [
        str(path)
        for path in pather.rglob("*")
        if path.is_file() and str(path).endswith(".mp4")
    ]
    dst_root = src_root
    if fps is not None:
    ...
    for i, src_path in tqdm.tqdm(
        enumerate(src_paths), desc=f"{src_root} -> {dst_root}"
    ):
        if i % env.num_tasks != env.global_rank:
            continue
        dst_path = src_path.replace(src_root, dst_root)
        ffmpeg_once(
            src_path, dst_path, fps=fps, resolution=resolution, pad=pad, mode=mode
        )
...
def distributed_encode(
    ...
    **kwargs,
    ...
    model = AutoModel.from_pretrained(
        vision_pretrained, device_map=f"cuda:{env.local_rank}"
    ).vision_model
    model.eval()
    dst_root = (
        f"{src_root}_{embed_mark.split('_')[-1]}_{vision_pretrained.replace('/', '--')}"
    )
    os.makedirs(dst_root, exist_ok=True)
    for i, file in tqdm.tqdm(
        enumerate(os.listdir(src_root)), desc=f"{src_root} -> {dst_root}"
    ):
        if i % env.num_tasks != env.global_rank:
            continue
        ...
        frames = torchvision.io.read_video(
            frame_path, pts_unit="sec", output_format="TCHW"
        )[0]
        with torch.no_grad():
            frames = torch.cat(
                [
                    vision_encode(model, batch.to(f"cuda:{env.local_rank}")).cpu()
...
def load_frames(path: str, start: float, end: float, num_threads=10) -> torch.Tensor:
    ...
    frames = torch.stack(
        [f["data"] for f in takewhile(lambda x: x["pts"] <= end, reader.seek(start))]
    )
