...
from ..utils import DictWithTo, ceil_time_by_fps, temporal_iou
...
class Ego4DNLQ(Ego4D):
    def __init__(self, split: str, **kwargs):
        ...
        for video_annos in annos["videos"]:
            ...
            for clip_annos in video_annos["clips"]:
                video_start_sec, video_end_sec = (
                    clip_annos["video_start_sec"],
                    clip_annos["video_end_sec"],
                )
                for _annos in clip_annos["annotations"]:
                    for query_anno in _annos["language_queries"]:
                        ...
                        query_start_time, query_end_time = (
                            query_anno["video_start_sec"],
                            query_anno["video_end_sec"],
                        )
                        regions = self.annos[sample_id]
                        merged = False
                        for region in regions:
                            if (
                                temporal_iou(region, [query_start_time, query_end_time])
                                > 0
                            ):
                                region = [
                                    min(region[0], query_start_time),
                                    max(region[1], query_end_time),
                                ]
                                merged = True
                                break
                        if not merged:
        ...
        self.annos = {
            k: sorted(query_region_times, key=lambda x: x[0])
            for k, query_region_times in self.annos.items()
        }
...
class Ego4DStreamNLQ(Ego4DNLQ, StreamMixIn):
    ...
    def __init__(self, split: str, frame_fps: int, **kwargs):
        ...
        for (
            ...
            video_end_time,
            query,
        ), query_region_times in tqdm.tqdm(self.annos.items()):
            ...
            video_start_time = ceil_time_by_fps(
                video_start_time, frame_fps, 0, video_end_time
            )
            ...
            for query_start_time, query_end_time in query_region_times:
                query_start_time = ceil_time_by_fps(
                    query_start_time,
                    frame_fps,
                    last_time + 1 / frame_fps,
                    video_end_time,
                )
                query_end_time = ceil_time_by_fps(
                    query_end_time, frame_fps, query_start_time, video_end_time
                )
                    ...
                    [
                        {
                            "role": "stream",
                            "num_frames": int(
                                (query_start_time - last_time) * frame_fps
                            ),
                            "learn": True,
                        },
                        {
                            "role": "assistant",
                            "content": f'The video related to the query "{query}" starts.',
                            "learn": True,
                        },
                        {
                            "role": "stream",
                            "num_frames": int(
                                (query_end_time - query_start_time) * frame_fps
                            ),
                            "learn": True,
                        },
                        {
                            "role": "assistant",
                            "content": f'The video related to the query "{query}" ends.',
                            "learn": True,
                        },
                    ]
                )
            ...
            annos.append(
                {
                    ...
                    "load_ranges": {
                        self.metadata[video_uid]["path"]: range(
                            int(video_start_time * frame_fps),
    ...
    def preprocess_conversation(self, conversation, query):
        query_prompt = random.choice(self.query_prompt_templates).replace(
            "QUERY", query
        )
    ...
    def __getitem__(self, index):
        ...
        return (
            *super().__getitem__(
                conversation=self.preprocess_conversation(
                    anno["conversation"], anno["query"]
                ),
