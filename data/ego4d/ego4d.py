...
class Ego4D:
    ...
    def __init__(
        self, vision_pretrained: str, embed_mark: str, frame_fps: int, **kwargs
    ):
        super().__init__(**kwargs)
        self.embed_dir = (
            f"{self.video_root}_{embed_mark}_{vision_pretrained.replace('/', '--')}"
        )
    ...
    def get_metadata(
        ...
        if os.path.exists(metadata_path):
            ...
            for file in tqdm.tqdm(
                os.listdir(self.embed_dir), desc=f"prepare {metadata_path}..."
            ):
