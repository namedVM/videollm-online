...
from ..utils import DictWithTo, ceil_time_by_fps, floor_time_by_fps, rand_bool
from .ego4d import Ego4D


class Ego4DGoalStepLiveChat(Ego4D, StreamMixIn):
    anno_path = (
        "datasets/ego4d/v2/annotations/goalstep_livechat_trainval_filtered_21k.json"
    )
        ...
        for anno in tqdm.tqdm(annos):
            ...
            if waiting_frames:
                conversation.append(
                    {
                        "role": "stream",
                        "num_frames": waiting_frames,
                        "learn": waiting_frames - 1,
                    }
                )
            conversation.append(
                {"role": "user", "content": content, "time": time, "fps_time": fps_time}
            )
            ...
            for message in anno["conversation"][1:]:
                ...
                if time == conversation[-1]["time"]:
                    if role == "user":
                            ...
                            conversation.append(
                                {
                                    ...
                                    "content": content,
                                    "time": time,
                                    "fps_time": conversation[-1]["fps_time"],
                                    "learn": True,
                                }
                            )
                        else:
                        ...
                        continue
                if role == "user":
                    fps_time = floor_time_by_fps(
                        time, frame_fps, conversation[-1]["fps_time"], duration
                    )
            ...
            self.annos.append(
                {
                    ...
                    "load_ranges": {
                        self.metadata[video_uid]["path"]: range(
                            int(start_fps_time * frame_fps),
                            int(conversation[-1]["fps_time"] * frame_fps) + 1,
                        )
                    },
                }
            )
    ...
    def preprocess_conversation(self, conversation):
        if self.augmentation and self.is_training and len(conversation) >= 4:  # 2 round
            ...
            ) - set(correct_assistant["content"])
            wrong_texts = list(wrong_texts) + [""]
            wrong_assistant = {
                "role": "assistant",
                "content": random.choice(wrong_texts),
            }
            ...
            if num_next_frames > 1:
                if rand_bool():  # promptly fix behavior
                    frame_placeholder_with_interval = (
                        self.v_placeholders_per_frame + self.frame_interval
                    )
                    next_stream_placeholder = frame_placeholder_with_interval * (
                        num_next_frames - 1
                    )
                    next_intervals = torch.arange(
                        len(frame_placeholder_with_interval),
                        len(next_stream_placeholder) + 1,
                        len(frame_placeholder_with_interval),
                    ) - len(self.frame_interval)
                    if self.frame_interval:  # last frame does not have frame interval
                        next_stream_placeholder = next_stream_placeholder[
                            : -len(self.frame_interval)
                        ]
                    augmented += [
                        {
                            "role": "stream",
                            "content": self.v_placeholders_per_frame,
                        ...
                        {
                            "role": "stream",
                            "content": next_stream_placeholder,
