...
from ..utils import DictWithTo, round_time_by_fps
...
class Ego4DLTA(Ego4D, StreamMixIn):
    ...
    def __init__(self, split: str, frame_fps: int, is_training: bool, **kwargs):
        super().__init__(
            frame_fps=frame_fps, is_training=is_training, split=split, **kwargs
        )
        self.split = split
        self.is_training = is_training
        # 1. build taxonomy
        taxonomy = json.load(
            open(os.path.join(self.root, "annotations", "fho_lta_taxonomy.json"))
        )
        ...
        for (i, verb), (j, noun) in itertools.product(
            enumerate(self.verbs), enumerate(self.nouns)
        ):
            action = f"{verb} {noun}"
            ...
            clip2anno[anno["clip_uid"]].append(
                {
                    ...
                    "start": anno["clip_parent_start_sec"]
                    + anno["action_clip_start_sec"],
                    "end": anno["clip_parent_start_sec"] + anno["action_clip_end_sec"],
                    "action_idx": anno["action_idx"],
                    "verb_label": anno.get("verb_label"),
                    "noun_label": anno.get("noun_label", None),
        ...
        for clip_uid, anno in clip2anno.items():
            for i in range(
                len(anno) - self.num_future_actions - self.num_input_actions + 1
            ):
                video_uid = anno[i]["video_uid"]
                j, k = (
                    i + self.num_input_actions,
                    i + self.num_future_actions + self.num_input_actions,
                )
                if "test_unannotated" in split:
                    verb_labels, noun_labels = None, None
                else:
                    verb_noun_labels = [
                        (a["verb_label"], a["noun_label"]) for a in anno[j:k]
                    ]
                    response = self.verb_noun_labels_to_text(verb_noun_labels)
                    verb_labels, noun_labels = zip(*verb_noun_labels)
                start_time = round_time_by_fps(
                    anno[i]["start"],
                    frame_fps,
                    min_time=0,
                    max_time=self.metadata[video_uid]["duration"],
                )
                end_time = round_time_by_fps(
                    anno[j - 1]["end"],
                    frame_fps,
                    min_time=0,
                    max_time=self.metadata[video_uid]["duration"],
                ...
                self.annos.append(
                    {
                        ...
                        "load_ranges": {
                            self.metadata[video_uid]["path"]: range(
    ...
    def verb_noun_labels_to_text(self, verb_noun_labels: list[tuple[str]]):
        return "\n".join(
            [
                f"{i + 1}. {self.verbs[v].capitalize()} {self.nouns[n]}."
                for i, (v, n) in enumerate(verb_noun_labels)
            ]
        )
    ...
    def map_action_to_verb_label(self, action: str):
        if action not in self.action_to_verb_label:
            action = min(
                [
                    (Levenshtein.distance(action, key), key)
                    for key in self.action_to_verb_label.keys()
                ]
            )[1]
    ...
    def map_action_to_noun_label(self, action: str):
        if action not in self.action_to_noun_label:
            action = min(
                [
                    (Levenshtein.distance(action, key), key)
                    for key in self.action_to_noun_label.keys()
                ]
            )[1]
    ...
    def text_to_verb_noun_ids(self, text: str, num_actions: int):
        ...
        verb_noun_ids = [
            (
                self.map_action_to_verb_label(action),
                self.map_action_to_noun_label(action),
            )
        ...
        if len(verb_noun_ids) < num_actions:
            if verb_noun_ids:
                verb_noun_ids = verb_noun_ids + [verb_noun_ids[-1]] * (
                    num_actions - len(verb_noun_ids)
                )
            else:
                verb_noun_ids = [
                    (
    ...
    def edit_distance(preds: np.ndarray, labels: np.ndarray):
        ...
        for n in range(N):
            dist = min(
                [editdistance.eval(preds[n, k, :], labels[n]) / Z for k in range(K)]
            )
            dists.append(dist)
    ...
    def AUED(preds, labels):
        ED = np.vstack(
            [
                Ego4DLTA.edit_distance(preds[:, :, :z], labels[:, :z])
                for z in range(1, Ego4DLTA.num_future_actions + 1)
            ]
        )
    ...
    def compute_metrics(
        ...
        **kwargs,
        ...
        batch_beam_pred_tensor, sample_idxs = (
        ...
        for beam_pred_tensor in batch_beam_pred_tensor:
            beam_pred_tensor = beam_pred_tensor[beam_pred_tensor != -100].reshape(
                self.num_beams, -1
            )
            beam_pred_string = tokenizer.batch_decode(
                beam_pred_tensor,
                skip_special_tokens=True,
                clean_up_tokenization_spaces=True,
            )
            beam_verb_noun_ids = np.array(
                [
                    self.text_to_verb_noun_ids(pred_string, self.num_future_actions)
                    for pred_string in beam_pred_string
                ]
            )  # 5 x 20 x 2
            beam_pred_verb_ids, beam_pred_noun_ids = (
                beam_verb_noun_ids[:, :, 0],
                beam_verb_noun_ids[:, :, 1],
            )
            batch_beam_pred_verb_ids.append(beam_pred_verb_ids)
            batch_beam_pred_noun_ids.append(beam_pred_noun_ids)
        batch_beam_pred_verb_ids, batch_beam_pred_noun_ids = (
            np.stack(batch_beam_pred_verb_ids),
            np.stack(batch_beam_pred_noun_ids),
        )
        if "test_unannotated" not in self.split:
            ...
            predictions = {}
            for beam_pred_verb_ids, beam_pred_noun_ids, sample_idx in zip(
                batch_beam_pred_verb_ids, batch_beam_pred_noun_ids, sample_idxs
            ):
                clip_uid = self.annos[sample_idx]["clip_uid"]
                last_visible_action_idx = self.annos[sample_idx][
                    "last_visible_action_idx"
                ]
                key = f"{clip_uid}_{last_visible_action_idx}"
                predictions[key] = dict(
                    verb=beam_pred_verb_ids.tolist(), noun=beam_pred_noun_ids.tolist()
                )
            if torch.cuda.current_device() == 0:
                json.dump(
                    predictions,
                    open(
                        os.path.join(output_dir, f"{self.split}_predictions.json"), "w"
                    ),
                )
            return {}
