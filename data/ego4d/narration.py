...
class Ego4DNarrationStream(Ego4D, StreamMixIn):
    ...
    instructions = [
        {
            "role": "user",
            "content": "Please concisely narrate the video in real time. Use the tag 'C' to denote the camera wearer, and other letter tags, such as 'X', to denote other individuals in the scene.",
        }
    ]
    ...
    def get_annos(self, split: str) -> dict[str, dict[str, list]]:
        annos = json.load(
            open(os.path.join(Ego4D.anno_root, "all_narrations_redacted.json"))
        )["videos"]
        assert split in ["train", "val", "test"]
        anno_path = os.path.join(
            Ego4D.anno_root, f"narration_stream_video_uids_{split}.json"
        )
        if os.path.exists(anno_path):
            ...
            for benchmark, key in tqdm.tqdm(
                Ego4DNarrationStream.benchmarks_with_keys.items(),
                desc=f"prepare {anno_path}",
            ):
                val_video_uids.extend(
                    [
                        anno["video_uid"]
                        for anno in json.load(
                            open(
                                os.path.join(
                                    Ego4D.root, "annotations", f"{benchmark}_val.json"
                                )
                            )
                        )[key]
                    ]
                )
                test_video_uids.extend(
                    [
                        anno["video_uid"]
                        for anno in json.load(
                            open(
                                os.path.join(
                                    Ego4D.root,
                                    "annotations",
                                    f"{benchmark}_test_unannotated.json",
                                )
                            )
                        )[key]
                    ]
            ...
            if split == "train":
                split_video_uids = list(
                    all_video_uids - val_video_uids - test_video_uids
                )
            elif split == "val":
        ...
        if os.path.exists(anno_path):
            ...
            for video_uid in tqdm.tqdm(
                split_video_uids, desc=f"prepare {anno_path}..."
            ):
                ...
                for narration in narrations:
                    _annotation_uid_narrations[narration.pop("_annotation_uid")].append(
                        narration
                    )
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.annos = []
        for video_uid, _annotation_uid_narrations in tqdm.tqdm(
            annos.items(), desc=f"narration_stream_{split}..."
        ):
            ...
            for narrations in _annotation_uid_narrations.values():
                ...
                start_time = ceil_time_by_fps(
                    narrations[0]["time"], frame_fps, min_time=0, max_time=duration
                )
                conversation = []
                ...
                for narration in narrations:
                    ...
                    if text == last_text:
                        continue
                    time = ceil_time_by_fps(
                        narration["time"], frame_fps, min_time=0, max_time=duration
                    )
                    if (
                        time == last_time
                    ):  # since we have sorted and ceiled, so directly replace, this time is more close
                ...
                self.annos.append(
                    {
                        ...
                        "load_ranges": {
                            self.metadata[video_uid]["path"]: range(
                                int(start_time * frame_fps),
                                int(last_time * frame_fps) + 1,
                            )
                        },
                    }
                )
    ...
    def preprocess_conversation(self, conversation):
        assert (
            conversation[0]["role"] == "stream" and conversation[0]["num_frames"] == 1
        )
        conversation[0]["learn"] = False
        return (
            conversation[:1] + [random.choice(self.instructions)] + conversation[1:]
        )  # first is stream
...
class Ego4DRefinedNarrationStream(Ego4DNarrationStream):
    instructions = [
        ...
        {
            "role": "user",
            "content": "Continuously answer what you observed with simple text.",
        },
        {"role": "user", "content": "Do concise real-time narration."},
        {
            "role": "user",
            "content": "Hey assistant, do you know the current video content? Reply me concisely.",
        },
        {"role": "user", "content": "Simply interpret the scene for me."},
        {"role": "user", "content": "What can you tell me about? Be concise."},
        {
            "role": "user",
            "content": "Use simple text to explain what is shown in front of me.",
        },
        {
            "role": "user",
            "content": "What is the action now? Please response in short.",
        },
    ]

    def get_annos(self, split: str) -> dict:
        anno_path = os.path.join(
            Ego4D.anno_root, f"refined_narration_stream_{split}.json"
        )
