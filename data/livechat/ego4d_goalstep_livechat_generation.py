...
class Ego4DLiveChatGeneration(Ego4D):
    ...
    def get_narrations(split):
        annos = []
        sources = json.load(
            open(os.path.join(Ego4D.anno_root, f"goalstep_{split}.json"))
        )["videos"]
                ...
                annos.append(
                    {
                        ...
                        "summary": (
                            source["start_time"],
                            source["end_time"],
                            source["goal_description"].strip(),
                        ),
                        "narrations": [
                            (
                                segment["start_time"],
                                segment["end_time"],
                                segment["step_description"].strip(),
                            )
    ...
    def __init__(
        self,
        num_queries_each_conversation: int,
        num_conversations_each_video: int,
        **kwargs,
    ):
        super().__init__(
            vision_pretrained=kwargs["vision_pretrained"],
            embed_mark=kwargs["embed_mark"],
            frame_fps=kwargs["frame_fps"],
        )
        ...
        annos = Ego4DLiveChatGeneration.get_narrations(
            "train"
        ) + Ego4DLiveChatGeneration.get_narrations("val")
        self.annos = []
        for anno in annos:
            ...
            timestamps = [
                ceil_time_by_fps(
                    t, self.frame_fps, 0, self.metadata[anno["video_uid"]]["duration"]
                )
                for t in timestamps
            ]
    ...
    def __call__(self, index):
        ...
        for nt in range(self.num_conversations_each_video):
            user_times = (
                (torch.rand(3) * (timestamps[-1] - timestamps[0]) + timestamps[0])
                .sort()
                .values.tolist()
            )
            user_times = [round(ut, 1) for ut in user_times]
            user_queries = random.sample(
                Templates.queries, self.num_queries_each_conversation
            )
            example = ""
            ...
            output_ids = self.model.generate(input_ids, max_length=8192)[
                :, input_ids.size(1) :
            ]
            ...
            try:
                ...
                for line in lines:
                    ...
                    anno["conversation"].append(
                        {"role": role.lower(), "content": content, "time": time}
                    )
                os.makedirs(f"{Ego4D.anno_root}/livechat/", exist_ok=True)
                json.dump(
                    anno,
                    open(
                        f"{Ego4D.anno_root}/livechat/{video_uid}_{index}_{nt}.json", "w"
                    ),
                    indent=4,
                )
            except:
...
if __name__ == "__main__":
    (args,) = HfArgumentParser(
        LiveOnePlusLiveChatGenerationArguments
    ).parse_args_into_dataclasses()
    executor = submitit.AutoExecutor(
        folder=f"outputs/preprocess/",
        cluster="local" if args.num_nodes == 1 else "slurm",
    )
