...
class COINBenchmark(COIN, StreamMixIn):
    ...
    def compute_metrics(
        ...
        batch_pred_tensor[batch_pred_tensor < 0] = (
            tokenizer.bos_token_id
        )  # not use clamp(min=0), since 0 is ! in Llama-3 tokenizer and may affect matching
        ...
        for prediction, label in zip(
            predictions, self.labels[sample_idxs]
        ):  # should be self.labels[sample_idx] to get the correct order
...
class COINStep(COINBenchmark):
    ...
    def __init__(self, *, split: str, frame_fps: int, is_training: bool, **kwargs):
        super().__init__(
            split=split, frame_fps=frame_fps, is_training=is_training, **kwargs
        )
        ...
        for anno in self._annos:
            ...
            for i in range(len(steps)):
                ...
                start_time = ceil_time_by_fps(
                    steps[i]["start"], frame_fps, min_time=0, max_time=duration
                )
                end_time = ceil_time_by_fps(
                    steps[i]["end"], frame_fps, min_time=0, max_time=duration
                )
...
class COINTask(COINBenchmark):
    ...
    def __init__(self, *, split: str, frame_fps: int, is_training: bool, **kwargs):
        ...
        for anno in self._annos:
            ...
            start_time = ceil_time_by_fps(
                anno["start"], frame_fps, min_time=0, max_time=duration
            )
            end_time = ceil_time_by_fps(
                anno["end"], frame_fps, min_time=0, max_time=duration
            )
            start_frame = int(start_time * frame_fps)
            end_frame = int(end_time * frame_fps) + 1
            conversation = [
            ...
            self.annos.append(
                {
                    "conversation": conversation,
                    "load_ranges": {
                        self.metadata[video_uid]["path"]: range(start_frame, end_frame)
                    },
                }
            )
        self.labels = np.array(self.labels)  # for fast indexing
        ...
        for anno in self._annos:
            ...
            for i in range(len(steps) - 1):
                ...
                if num_next_steps == 1:
                    ...
                    response = "\n".join(
                        f"{i + 1}. {s['text'].capitalize()}."
                        for i, s in enumerate(next_steps)
                    )
                    self.labels.append(
                        np.array([s["text"].lower() for s in next_steps])
                    )
                conversation.append(
                    {"role": "assistant", "content": response, "learn": True}
                )
