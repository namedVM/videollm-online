...
def main(output_dir, json_path, num_workers, ffmpeg_location):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=num_workers) as executor:
        futures = [
            ...
            for video_id, annotation in annotations
        ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download videos in parallel using yt-dlp"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="datasets/coin/videos",
        help="Directory to save downloaded videos",
    )
    parser.add_argument(
        "--json_path",
        type=str,
        default="datasets/coin/coin.json",
        help="Path to the JSON file containing video data",
    )
    parser.add_argument("--ffmpeg", type=str, default=None)
    parser.add_argument(
        "--num_workers", type=int, default=16, help="Number of parallel downloads"
    )
